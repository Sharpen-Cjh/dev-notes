...
## 우리 자체 JWT 검증 — "이 요청, 로그인된 사용자가 보낸 게 맞나"

로그인 API들은 토큰을 **발급(서명)**하는 쪽이었다면, 이후의 모든 API(공유, 사진 등)는 그 토큰을 **검증**해서 "누가 요청했는지" 알아내야 한다. `signJWT`의 반대 동작.
...
async function getUserId(request: Request, env: Env): Promise<string | null> {
	...
	const [header, payload, signature] = authHeader.slice(7).split(".");
	// ... 서명을 다시 계산해서 signature와 일치하는지 검증 ...
	// ... payload를 디코드해서 exp(만료시각) 지났는지 확인 ...
	return decoded.sub ?? null;
}
```

- **`Authorization: Bearer <토큰>`**: 클라이언트가 인증된 요청을 보낼 때 관례적으로 쓰는 HTTP 헤더 형식. "Bearer"(소지자)는 "이 토큰을 가진 사람이라면 누구든"이라는 뜻 — 토큰 자체가 곧 자격 증명이다.
- 검증에 실패하면(서명 불일치, 만료, 헤더 없음) `null`을 돌려주고, 호출하는 쪽에서 401(로그인 필요) 처리.
- 이 함수를 모든 보호된 엔드포인트 맨 앞에서 호출하는 패턴이 반복된다 — 나중에 라우팅 프레임워크(Hono 등)를 쓰면 이런 반복을 "미들웨어"로 한 번만 선언해도 되지만, 지금은 각 핸들러 안에서 직접 호출.
...
## 카카오 앱 키가 여러 종류인 이유 — "이 코드가 어디서 실행되는가"

카카오 API를 부를 때마다 "이 요청이 진짜 등록된 내 앱에서 온 게 맞는지" 인증해야 하는데, 인증 방식이 **요청이 오는 실행 환경**에 따라 다르다. 그래서 키가 4종류로 나뉜다.

- **REST API 키**: 백엔드 서버에서 카카오 API를 호출할 때. 서버 코드 안에만 있고 사용자에게 안 보이는 환경이라 상대적으로 안전 — 필요하면 클라이언트 시크릿까지 같이 써서 이중으로 잠글 수도 있다.
- **JavaScript 키**: 브라우저(웹 프론트엔드)에서 카카오 SDK(JS)를 쓸 때. 브라우저 소스는 누구나 볼 수 있어 키 노출이 전제 조건 — 대신 "등록된 도메인에서만 동작"하는 것으로 방어한다.
- **네이티브 앱 키**: 모바일(Android/iOS) 앱용. APK 안에 그대로 박혀 배포되니 이것도 노출 전제인데, 도메인 대신 **키 해시**(앱 서명 인증서를 해시한 값)로 "진짜 내가 서명한 그 APK에서 온 요청인지" 검증한다. 판도라의 상자 앱이 쓰는 게 이 키.
- **어드민 키**: 유저 강제 탈퇴, 메시지 강제 발송처럼 앱을 관리자 권한으로 조작하는 초강력 키. 절대 클라이언트/앱에 넣지 않고 극히 제한된 서버 안에서만 쓴다.

기준을 한 줄로: 서버(안 보임, 안전) < 앱(보이지만 서명으로 검증 가능) < 브라우저(다 보임, 도메인으로 방어) < 어드민(뭐든 다 함, 제일 위험) 순으로 노출 위험이 커진다.

**우리 백엔드는 REST API 키를 안 쓴다**는 점이 특이한데, 카카오 로그인 검증 방식 자체가 "유저가 앱에서 받은 accessToken을 그대로 카카오 서버에 넘겨서 유효한지만 물어보는" 방식(바로 위 섹션)이라, 서버가 자기 자신을 증명할 키가 따로 필요 없기 때문이다.
