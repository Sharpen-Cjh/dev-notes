...
**스트림**은 코틀린만의 개념이 아니라, **"데이터가 한 번에 다 오는 게 아니라 시간이 지나면서 여러 번에 걸쳐 오는 것"**을 가리키는 일반적인 개념이다.

이미 익숙한 단어이기도 하다 — 넷플릭스/유튜브의 "스트리밍(streaming)"이 같은 단어다. 영화를 볼 때 파일 전체를 다운로드한 뒤 재생하는 게 아니라, 재생하면서 동시에 데이터가 조금씩 계속 흘러들어온다. "영화 전체"라는 하나의 큰 덩어리가 아니라, 시간에 걸쳐 조각조각 여러 번 온다는 게 핵심이다.

프로그래밍에서도 마찬가지다: 파일을 조금씩 읽어오는 것도 스트림, 버튼을 여러 번 누르는 클릭 이벤트들도 스트림, 넷플릭스 영상 데이터도 스트림 — 전부 "시간에 걸쳐 여러 번 오는 데이터"라는 같은 범주에 속한다.

**`Flow`는 코틀린이 이 "스트림" 개념을 실제 코드로 다룰 수 있게 만든 구체적인 도구(타입)다.**

- 스트림 = 흐르는 것이라는 개념 자체 (강물, 스트리밍 영상, 클릭 이벤트 등을 다 포함하는 큰 범주)
- `Flow` = 코틀린에서 그 개념을 다루려고 만든 구체적인 도구 하나 (자바에는 다른 스트림 도구가 있고, RxJava에도 또 다른 스트림 도구가 있다 — `Flow`는 그중 코틀린 코루틴 진영의 것)

그래서 Room의 Dao가 `Flow<List<PhotoEntity>>`를 리턴한다는 건, "photos 테이블 내용이 스트림(시간에 걸쳐 여러 번 흘러나오는 데이터)으로 온다"는 뜻이고, 테이블이 바뀔 때마다 새 목록이 그 스트림을 타고 흘러나온다.
