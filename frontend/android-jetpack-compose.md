...
컴포저블 하나에 "크기/여백/배경/테두리/클릭 가능 여부/정렬" 같은 설정을 붙이는 꾸러미. `Modifier` 자체는 "아무 설정 없음"인 빈 값이고, 여기에 확장 함수를 체이닝해서 설정을 하나씩 추가한다.
...
Modifier
    ...
    .border(3.dp, RetroBrown)
```

각 `.함수()`는 "이전 설정 + 새 설정"이 합쳐진 **새 Modifier**를 리턴하는 [확장 함수](../language/kotlin.md)라서 계속 이어 붙일 수 있다. **순서가 결과에 영향을 준다** — 예를 들어 `padding`을 `background`보다 먼저 주면 배경이 여백 안쪽에만 칠해지고, 나중에 주면 여백까지 칠해진다.

- `fillMaxWidth()`: 부모가 허용하는 가로 폭 전체를 채운다. 세로는 내용물 크기만큼.
- `fillMaxSize()`: 가로+세로 둘 다 부모의 최대치로 채운다.
- `height(320.dp)`처럼 특정 축만 고정값으로 줄 수도 있고, `fillMaxWidth()`랑 섞어서 "가로는 꽉 채우고 세로는 고정"도 가능하다.
